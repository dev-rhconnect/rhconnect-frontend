...
export default function EmploiDuTempsPage() {
  ...
  const disposFiltrees = contratId !== ''
    ? disponibilites.filter((d) => {
        const c = contrats.find((ct) => ct.id === contratId)
        return c ? d.vacataireId === c.vacataireId : true
      })
    : disponibilites

  const canSubmit = contratId !== '' && dateSeance && heureDebut && heureFin
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-end gap-3 border-t border-gray-100 px-6 py-4">
              ...
              <button
                ...
                className="rounded-xl px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50 transition-colors"
                style={{ background: '#C88500' }}
              >
                {creating ? 'Planification…' : 'Planifier la séance'}
              </button>
            </div>
          </div>
        </div>
